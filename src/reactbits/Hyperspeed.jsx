// React Bits — Hyperspeed (TypeScript + Tailwind), ported to JSX.
// Dependencies: three, postprocessing.
// The component fills its parent container. Memoize `effectOptions`.
  ...
  xyDistortion: {
    ...
    getDistortion: `
      ...
      vec3 getDistortion(float progress){
        ...
        return vec3(
          cos(progress * PI * uFreq.x + uTime) * uAmp.x - cos(movementProgressFix * PI * uFreq.x + uTime) * uAmp.x,
          sin(progress * PI * uFreq.y + PI/2. + uTime) * uAmp.y - sin(movementProgressFix * PI * uFreq.y + uTime) * uAmp.y,
          0.
        );
      }
    `,
...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS, lightMode = false }) => {
  ...
  useEffect(() => {
    ...
    // WebGL can be unavailable or the context can be lost (sandboxed iframes,
    // GPU blocklists, tab suspension). Fail soft: the page keeps rendering
    // without the road instead of throwing and unmounting the app.
    const failSoft = (err) => {
      console.warn('[Hyperspeed] WebGL unavailable, skipping background:', err);
      if (appRef.current) {
        try {
          appRef.current.dispose();
        } catch (e) {
    ...
    try {
      ...
      const canvas = myApp.renderer.domElement;
      const onContextLost = (e) => {
        e.preventDefault();
        failSoft(new Error('WebGL context lost'));
      };
      ...
      myApp.loadAssets().then(() => {
        if (disposed) return;
        try {
          if (!myApp.renderer.getContext()) throw new Error('WebGL context unavailable');
          myApp.init();
        } catch (err) {
          failSoft(err);
