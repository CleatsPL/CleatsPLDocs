// React Bits — Topography (TypeScript + Tailwind), ported to JSX.
// Dependency: ogl.
//
// Adjustments for this project:
//  - shader fix: single-argument max() in the lightMode path is invalid GLSL
//    (it broke program compilation in every browser) — now max(r, max(g, b))
//  - fail-soft WebGL init: if the context is unavailable or rendering throws
//    (sandboxed iframes, GPU blocklists, lost contexts) the component degrades
//    to an empty transparent layer instead of crashing the React tree.
...
const Topography = ({
  ...
  useEffect(() => {
    ...
    const failSoft = (err) => {
      ...
      failed = true;
      console.warn('[Topography] WebGL unavailable, skipping background:', err);
      cleanupAll();
    };
    ...
    try {
      ...
      const tryStart = () => {
        if (!failed && !disposed && isVisible && isPageVisible && raf === 0) raf = requestAnimationFrame(loop);
      };
      const tryStop = () => {
        if (raf !== 0) {
